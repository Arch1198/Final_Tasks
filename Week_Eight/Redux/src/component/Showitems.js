...
import { ProdContext } from './context/ProductListContext'
import { NavLink } from "react-router-dom";
const ShowItem = (props) => {
  const [products, setProducts] = useState(ItemList); 
  const dataObj=useContext( ProdContext)
  useEffect(()=>{
    if(dataObj.spinState===false)
      console.log(dataObj.prodList)
  },[dataObj])
  ...
  const DeleteItemFromProduct = (Id) => {
    ...
    dataObj.prodList.splice(Id,1)
  }; 
  if(dataObj.spinState===true){
    return(
    <>
 <Spinner/>
    </>
  ...
  return (
    <>
      ...
      >
        {dataObj.prodList.map((val, index) => {
          return (
            <Card
